...
% ---------------------------------------------------------------------------
% Cisco-style CLI examples using verbatim text inside a light highlighted box
% ---------------------------------------------------------------------------
\newtcolorbox{cliexample}[1]{
	breakable,
	 enhanced,
	colback=ciscolightgray,
	colframe=ciscoblue,
	boxrule=0.7pt,
	arc=1pt,
	left=8pt,
...
\begin{document}
	...
	\begin{LTR}
		\begin{cliexample}{Command / Response}
			\begin{verbatim}
	...
	در این پیوست، نحوهٔ به‌روزرسانی سیستم‌عامل (\lr{Firmware}) سوییچ شرح داده شده است. در حال حاضر، این فرایند از طریق منوی \lr{Boot}، اتصال مستقیم شبکه و سرور \lr{TFTP} انجام می‌پذیرد؛ شایان ذکر است در نسخه‌های آینده، امکان به‌روزرسانی از طریق محیط خط فرمان (\lr{CLI}) نیز فراهم خواهد شد.

	نام ایمیج (\lr{Image}) استفاده‌شده در این سند، \lr{mipsel\_vsc7429.mfi} است. پیش از اقدام به اجرا، حتماً سازگاری آن با مدل و نسخهٔ سخت‌افزار دستگاه را از طریق منبع رسمی بررسی و تأیید نمایید.
...
برای ورود به بخش \lr{Boot} و آماده‌سازی سوییچ جهت به‌روزرسانی، مراحل زیر را دنبال کنید:

	\begin{enumerate}
		\item ابتدا سوییچ را ریست (\lr{Reboot}) نمایید.
		\item در محیط کنسول، با مشاهده شمارندهٔ معکوس زمان (همان‌طور که در تصویر مشخص شده است)، کلید \lr{Enter} را فشار دهید تا فرایند بوت متوقف شده و وارد تنظیمات \lr{Bootloader} شوید.
		\begin{flushleft}
			\includegraphics[width=10cm]{Luton.png}
		\end{flushleft}

		\item در این حالت، خط فرمان (پرامپت) به صورت \lr{\texttt{Luton\#}} تغییر می‌یابد.
		\item سپس با استفاده از دستور \lr{\texttt{ping}}، برقراری ارتباط شبکه را بررسی نمایید.
	\end{enumerate}
